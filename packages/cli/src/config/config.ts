...
import {
  ...
  setProdigyMdFilename as setServerGeminiMdFilename,
  getCurrentProdigyMdFilename,
  ApprovalMode,
...
export async function parseArguments(): Promise<CliArgs> {
  const yargsInstance = yargs(hideBin(process.argv))
    ...
    .usage(
      '$0 [options]',
      'Prodigy CLI - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('model', {
      alias: 'm',
      type: 'string',
...
export async function loadCliConfig(
  ...
  if (settings.contextFileName) {
    ...
    setServerGeminiMdFilename(getCurrentProdigyMdFilename());
  }

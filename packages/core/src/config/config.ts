...
import {
  MemoryTool,
  setProdigyMdFilename,
  PRODIGY_CONFIG_DIR as PRODIGY_DIR,
...
export class Config {
  ...
  private prodigyMdFileCount: number = 0;
  ...
  constructor(params: ConfigParameters) {
    ...
    this.prodigyMdFileCount = params.llxprtMdFileCount ?? 0;
    ...
    if (params.contextFileName) {
      setProdigyMdFilename(params.contextFileName);
    }
  ...
  getLlxprtMdFileCount(): number {
    return this.prodigyMdFileCount;
  }

  setLlxprtMdFileCount(count: number): void {
    this.prodigyMdFileCount = count;
  }
  ...
  getGeminiDir(): string {
    return path.join(this.targetDir, PRODIGY_DIR);
  }

...
export const PRODIGY_CONFIG_DIR = '.prodigy';
export const DEFAULT_CONTEXT_FILENAME = 'PRODIGY.md';
export const MEMORY_SECTION_HEADER = '## Gemini Added Memories';

// Legacy exports for backward compatibility
export const setLlxprtMdFilename = setProdigyMdFilename;
export const getCurrentLlxprtMdFilename = getCurrentProdigyMdFilename;
export const getAllLlxprtMdFilenames = getAllProdigyMdFilenames;
export const LLXPRT_CONFIG_DIR = PRODIGY_CONFIG_DIR;

// This variable will hold the currently configured filename for PRODIGY.md context files.
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setProdigyMdFilename.
let currentProdigyMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setProdigyMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentProdigyMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentProdigyMdFilename = newFilename.trim();
  }
}

export function getCurrentProdigyMdFilename(): string {
  if (Array.isArray(currentProdigyMdFilename)) {
    return currentProdigyMdFilename[0];
  }
  return currentProdigyMdFilename;
}

export function getAllProdigyMdFilenames(): string[] {
  if (Array.isArray(currentProdigyMdFilename)) {
    return currentProdigyMdFilename;
  }
  return [currentProdigyMdFilename];
...
function getGlobalMemoryFilePath(): string {
  return path.join(homedir(), PRODIGY_CONFIG_DIR, getCurrentProdigyMdFilename());
}

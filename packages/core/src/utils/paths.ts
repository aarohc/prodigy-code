...
export const PRODIGY_DIR = '.prodigy';
export const GOOGLE_ACCOUNTS_FILENAME = 'google_accounts.json';

// Legacy exports for backward compatibility
export const LLXPRT_DIR = PRODIGY_DIR;
export const ensureLlxprtDirExists = ensureProdigyDirExists;
const TMP_DIR_NAME = 'tmp';
const COMMANDS_DIR_NAME = 'commands';

export function ensureProdigyDirExists() {
  const homeDir = os.homedir();
  const prodigyDir = path.join(homeDir, PRODIGY_DIR);
  if (!fs.existsSync(prodigyDir)) {
    fs.mkdirSync(prodigyDir, { recursive: true });
  }
}
...
export function getProjectTempDir(projectRoot: string): string {
  ...
  return path.join(os.homedir(), PRODIGY_DIR, TMP_DIR_NAME, hash);
}
...
export function getUserCommandsDir(): string {
  return path.join(os.homedir(), PRODIGY_DIR, COMMANDS_DIR_NAME);
}
...
export function getProjectCommandsDir(projectRoot: string): string {
  return path.join(projectRoot, PRODIGY_DIR, COMMANDS_DIR_NAME);
}
